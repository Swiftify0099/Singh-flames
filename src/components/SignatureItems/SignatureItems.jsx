...
const SignatureItems = ({
  ...
  const signatures = getItemsByCategory("Signatures");
...
const SignatureCard = ({ pizza, index, sizes, addToOrder }) => {
    ...
    >
      ...
      <div className="space-y-2 mb-4">
        {sizes.map((size) => (
          <label
            key={size.id}
            className={`flex items-center justify-between cursor-pointer rounded-lg border p-2 transition-colors ${
